...
function onCollisionEnter(collision_info)
  ...
  if allowableTokens[newTokenName] != nil then
...
function addToken(tokenName)
  if allowableTokens[tokenName] != nil and IS_UNIT_LEADER then
    allowableTokens[tokenName].state = true
    allowableTokens[tokenName].quantity = allowableTokens[tokenName].quantity + 1
...
function _updateTokenDisplay(tokenName)
  ...
  if count <= 0 then
    allowableTokens[tokenName].state = false
